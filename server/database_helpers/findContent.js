...
//const uri = "mongodb+srv://pglenn:NuJx4euHxZesb9dH@lostarksoloproject.imnzb4e.mongodb.net/?retryWrites=true&w=majority"
...
async function findBestContent(ilvl,contentType) {
  ...
  try {
    ...
    const contentList = await client.db('dungeon_and_character_data').collection(contentType).find().sort('ilvl').toArray(); 
    ...
    if( contentType === 'chaos_dungeons'|| contentType=== "guardian_raids" ) {
      ...
      for ( let i = contentList.length-1 ; i >= 0 ; i-- ) {
        ...
        if(contentList[i].ilvl <= ilvl && bestContent.length < 3 ) {
          ...
          let canBeAdded = bestContent.reduce( (isOK, c) => isOK && (c.name !== contentList[i].name), true  ); 
