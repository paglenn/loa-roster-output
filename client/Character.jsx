...
const Character = (props) => {
 ...
 return (
  <div className={`${styles.characterCard} ${isGoldEarner ? styles.gold : ''}` }> 
    ...
    <div> Name: {name} <img className={imageStyles.classIcon} src={icons[classNameIcon]} /> </div>

    {/* Character item level container  */}
    <div className={styles.ilvl}> 
        {/* Item level display and updating  */}
      <span>Item Level:  <b>{ilvl} </b> </span>
      {/* Form to update item level  */}
      <form onSubmit={props.handleLevelUpdate} name={`${name}.${isGoldEarner}`}>
    ...
    {/* Class Picture  */}
      <div className={styles.imgContainer}> <img className={`${imageStyles.CharModel}  ${isGoldEarner ? imageStyles.gold : ''}`} src={classImages[_class.toLowerCase()]}/> </div>
      ...
      <div>
        <div> 
          ...
          <input type="checkbox" name={`${name}.${ilvl}`} checked={ isGoldEarner} onChange={props.handleGoldUpdate}/>
        </div>
        <div><img src={icons.silverIcon} alt={`${name} silver`}/> {Math.round(resources.silver)}</div>
        <div><img className={imageStyles.gem} src={icons.gemIcon} alt={`${name} gems `}/> {Math.round(resources.gems)}</div>
